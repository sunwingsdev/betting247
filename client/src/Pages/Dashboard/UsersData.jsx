...
const UsersData = () => {
  ...
  const filteredUsers = users?.filter((selectedUser) =>
    user?.role === "mother-admin"
      ? (selectedUser?.role ? selectedUser?.role === role : "user" === role) &&
        (selectedUser?.createdBy === user?._id ||
          selectedUser?.createdBy === "self")
      : (selectedUser?.role ? selectedUser?.role === role : "user" === role) &&
        selectedUser?.createdBy === user?._id
  );
        ...
        <div className="overflow-x-auto ">
          <table className="min-w-full divide-y divide-gray-300 text-center">
            <thead className="bg-headerGray text-headingTextColor">
              <tr className="border bg-yellow-400 border-gray-300 px-4 py-2 text-sm font-medium uppercase">
                <th className="border border-black bg-headerGray  px-4 py-2 text-sm font-medium uppercase">
                  Username
                </th>
                <th className="border border-black bg-headerGray  px-4 py-2 text-sm font-medium uppercase">
                  Full Name
                </th>
                <th className="border border-black bg-headerGray  px-2 py-2 text-sm font-medium uppercase">
                  Email
                </th>
                <th className="border border-black bg-headerGray  px-4 py-2 text-sm font-medium uppercase">
                  Balance
